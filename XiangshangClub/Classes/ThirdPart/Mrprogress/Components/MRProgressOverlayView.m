...
@property (nonatomic, strong, readwrite) UILabel *titleLabel; //将之前的weak改为strong
...
//    if (CurrentIOS7) {
   ...
   // if (CurrentIOS7) {
    ...
    self.titleLabel.attributedText = [[NSAttributedString alloc] initWithString:@" " attributes:@{
...
//    modeView.backgroundColor = XSTest_Color;
...
- (void)manualLayoutSubviews {
    ...
    if (!hasSmallIndicator) {
        ...
        if (self.mode != MRProgressOverlayViewModeDeterminateHorizontalBar) {
            modeViewFrame = CGRectMake(modePadding, y, innerViewWidth, CGRectGetHeight(self.modeView.frame));
            paddingBottom = 20;
        } else {
